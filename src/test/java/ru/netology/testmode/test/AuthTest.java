...
class AuthTest {
    ...
    void shouldGetErrorIfNotRegisteredUser() {
        ...
        $("[data-test-id='error-notification'] .notification__content")
                .shouldHave(Condition.exactText("Ошибка! Неверно указан логин или пароль"), Duration.ofSeconds(5000))
    ...
    void shouldGetErrorIfBlockedUser() {
        ...
        $("[data-test-id='action-login']").click();
        $("[data-test-id='error-notification'] .notification__content")
                .shouldHave(Condition.exactText("Ошибка! Пользователь заблокирован"), Duration.ofSeconds(5000))
                .shouldBe(Condition.visible);
    }
    ...
    void shouldGetMessageFieldMustBeFilledInShouldAppearLogin() {
        ...
        $("[data-test-id='login'] .input__sub")
                .shouldHave(Condition.exactText("Поле обязательно для заполнения"), Duration.ofSeconds(5000))
                .shouldBe(Condition.visible);
    }
